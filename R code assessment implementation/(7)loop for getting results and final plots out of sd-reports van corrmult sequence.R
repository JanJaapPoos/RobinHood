...
for(i in seq(1,1,0.05)){
  ...
  endplot<-ggdraw() +
    draw_plot(U_plot_polite_rh, y = 0, x = 0.245, width = 0.245, height = 1) +
    draw_plot(B_plot_polite_rh, y= 0, x = 0, width = 0.245, height = 1)+
    draw_plot(C_plot_polite_rh, y = 0, x = 0.495,width = 0.245, height = 1)+
    draw_plot(I_plot_polite_rh, y = 0, x = 0.745, width = 0.245, height = 1) 
  ...
  I_plot_polite_combi<-ggplot(data = indices_combined) +
    ...
    theme_bw()+
    scale_colour_manual(values = col_values_7)+#,guide=F
    geom_line(aes(x=years,y=as.numeric(indices_combined$est_indices_rh),linetype="22", colour=species.1),linetype="22",alpha=0.8, size=0.75,show.legend = F)+
    geom_line(aes(x=years,y=as.numeric(indices_combined$est_indices),linetype="22", colour=species.1),linetype="solid",alpha=0.8, size=0.75,show.legend = F)+
    geom_ribbon(aes(x=years,ymin=as.numeric(indices_combined$Min_rh), ymax=as.numeric(indices_combined$Max_rh),fill=species.1),alpha=0.4)+
    geom_ribbon(aes(x=years,ymin=as.numeric(indices_combined$Min), ymax=as.numeric(indices_combined$Max),fill=species.1),alpha=0.2)+
    scale_alpha_manual(guide=F)+
  ...
  BoBMSY_plot_polite_combi<-ggplot(data=BoBMSYhat_combined)+
    ylab("B/BMSY")+
    xlab("Years")+
    geom_line(aes(x = years, y = as.numeric(BoBMSYhat_combined$BoBMSY),linetype="dashed",colour=species.1),linetype="solid",size=0.75, alpha=0.8,show.legend=F)+
    geom_line(aes(x = years, y = as.numeric(BoBMSYhat_combined$BoBMSY_rh),linetype="dashed",colour=species.1),linetype="22",size=0.75, alpha=0.8,show.legend=F)+
    facet_wrap(facets = vars(species.1),nrow=1)+#,scales="free_y"
    theme_bw()+
    geom_hline(aes(yintercept=1,colour=species.1),linetype="twodash", size=0.8, alpha=0.4 )+
    scale_colour_manual(values = col_values_5,guide=F)+
    geom_ribbon(aes(x=years, ymin=as.numeric(BoBMSYhat_combined$Min), ymax=as.numeric(BoBMSYhat_combined$Max),fill=species.1),linetype="22",alpha=0.2)+
    geom_ribbon(aes(x=years, ymin=as.numeric(BoBMSYhat_combined$Min_rh), ymax=as.numeric(BoBMSYhat_combined$Max_rh),fill=species.1),linetype="22",alpha=0.4)+
    scale_alpha_manual(guide=F)+
    scale_fill_manual(values = col_values_5,guide=F)+
    #scale_linetype_manual(name="data type", values=c("22",""), labels=c("estimates",""))+
    #ggtitle("Estimates of B/BMSY")+
  ...
  FoFMSY_plot_polite_combi<-ggplot(data=FoFMSYhat_combined)+
    ylab("F/FMSY")+
    xlab("Years")+
    geom_line(aes(x = years, y = as.numeric(FoFMSYhat_combined$FoFMSY),linetype="dashed",colour=species.1),linetype="solid",size=0.75, alpha=0.8,show.legend=F)+
    geom_line(aes(x = years, y = as.numeric(FoFMSYhat_combined$FoFMSY_rh),linetype="dashed",colour=species.1),linetype="22",size=0.75, alpha=0.8,show.legend=F)+
    facet_wrap(facets = vars(species.1),nrow =1)+#,scales="free_y"
    theme_bw()+
    geom_hline(aes(yintercept=1,colour=species.1),linetype="twodash", size=0.8, alpha=0.4 )+
    scale_colour_manual(values = col_values_5,guide=F)+
    geom_ribbon(aes(x=years, ymin=as.numeric(FoFMSYhat_combined$Min), ymax=as.numeric(FoFMSYhat_combined$Max),fill=species.1),linetype="22",alpha=0.2)+
    geom_ribbon(aes(x=years, ymin=as.numeric(FoFMSYhat_combined$Min_rh), ymax=as.numeric(FoFMSYhat_combined$Max_rh),fill=species.1),linetype="22",alpha=0.4)+
    scale_alpha_manual(guide=F)+
    scale_fill_manual(values = col_values_5,guide=F)+
    #scale_linetype_manual(name="data type", values=c("22",""), labels=c("estimates",""))+
    #ggtitle("Estimates of F/FMSY")+
